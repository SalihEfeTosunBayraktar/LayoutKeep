...
REM
REM Short by design: 12 chunks x 4 pages keeps the run near half an hour on the local model,
...
.venv\Scripts\python.exe tools\audit\translate_book.py ^
  ...
  --name gutenberg_math --to tr --model google/gemma-4-e4b --workers 2 ^
  --pages-per-chunk 4 --limit-chunks 12 --layout-detector --resume ^
